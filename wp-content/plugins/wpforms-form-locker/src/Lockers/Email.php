...
class Email extends Locker {
	...
	/**
	 * Identify where the form is locked/unlocked.
	 *
	 * @since 2.0.0
	 *
	 ...
	 * @return bool
	 */
	public function display_form( $load_form, $form_data ) { // phpcs:ignore WPForms.PHP.HooksMethod.InvalidPlaceForAddingHooks
	...
	/**
	 * Check if a form is locked by conditions.
	 *
	 * @since 2.0.0
	 *
	 * @return bool
	...
	public function locked_html() {
		...
		wpforms()->obj( 'frontend' )->forms[ $locked_id ]       = $this->form_data;
		wpforms()->obj( 'frontend' )->forms[ $locked_id ]['id'] = $locked_id;
		...
		?>
			...
			if ( ! empty( $email_sent_message ) ) {
				...
				?>
				<form id="wpforms-form-<?php echo esc_attr( $locked_id ); ?>"
					class="wpforms-validate wpforms-form"
					data-formid="<?php echo esc_attr( $locked_id ); ?>"
					method="post"
					enctype="multipart/form-data"
					action="<?php echo esc_attr( $action ); ?>"
					aria-invalid="true"
					aria-errormessage="wpforms-form-<?php echo esc_attr( $locked_id ); ?>-error">
					...
					<div class="wpforms-field-container">
						<div id="wpforms-<?php echo esc_attr( $locked_id ); ?>-field_form_locker_email-container" class="wpforms-field wpforms-field-email" data-field-id="form_locker_email" data-field-type="form_locker_email">
							<label class="wpforms-field-label" for="wpforms-<?php echo esc_attr( $locked_id ); ?>-field_form_locker_email">
								...
								<span class="wpforms-required-label" aria-label="<?php esc_attr_e( 'Required', 'wpforms-form-locker' ); ?>">*</span>
							</label>
	...
	/**
	 * Get a locked form message from an admin area.
	 *
	 * @since 2.0.0
	 *
	 * @return string
	...
	/**
	 * Display successfully sent a message after an email verification link is sent.
	 *
	 * @since 2.0.0
	 *
	 * @return string
	...
	public function add_entry() {
		...
		$row     = wpforms_form_locker()->email_storage->get_form_locker_row(
			[
				'form_id' => $form_id,
				'email'   => $email,
			]
		...
		// Prevent multiple email sending.
		unset(
			$_POST['wpforms']['form_locker_email'],
			$_POST['wpforms']['form_locker_form_id']
		);
	}
	...
	public function get_email_verified() {
		...
		if ( is_array( $token ) && count( $token ) === 2 ) {
			$has_record = wpforms_form_locker()->email_storage->get_form_locker_row(
				[
					'form_id' => $token[0],
					'email'   => $token[1],
				]
	...
	/**
	 * Add an email field to the form to process locked form as normal.
	 *
	 * @since 2.0.0
	 */
	...
	/**
	 * Successfully sent a confirmation message.
	 *
	 * @since 2.3.0
	 *
