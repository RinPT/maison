...
class UserEntryLimit extends Locker {
	...
	private function get_total_by_email() {
		...
		$fields_table_name      = wpforms()->obj( 'entry_fields' )->table_name;
		$entries_table_name     = wpforms()->obj( 'entry' )->table_name;
		$payments_table_name    = wpforms()->obj( 'payment' )->table_name;
		...
		$select = "
			...
			LEFT JOIN $payments_table_name ON $entries_table_name.entry_id = $payments_table_name.entry_id
		";
		...
		$where[] = $wpdb->prepare( "AND $fields_table_name.field_id = %s",  $field_id );
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$where[] = $wpdb->prepare( "AND $fields_table_name.value = %s", $email );
	...
	private function get_total_by_ip() {
		...
		$select = "
			SELECT $entries_table_name.entry_id AS entry_id
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$where[] = $wpdb->prepare( "AND $entries_table_name.form_id = %d",  $this->form_data['id'] );
		$where   = $this->compose_sql_status( $where );
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$where[] = $wpdb->prepare( "AND $entries_table_name.ip_address = %s", $ip_address );
	...
	private function compose_sql_status( $where ) {
		...
		if ( $excluded ) {
			$where[] = "AND $entries_table.status NOT IN ('$excluded')";

			$where[] = "AND ($payments_table.entry_id IS NULL OR (
