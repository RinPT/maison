...
### Fixed
- Error message for the required and unique Name and Phone fields sometimes disappeared.
- Closed message wasn't shown when the Total Entry Limit was reached.
...
- Email verification could end up sending multiple emails due to a conflict with some plugins.
- 500 Internal Server Error occurred when there was a huge number of entries for a form with the Unique Answers setting.
- Now unique answers are supported in the Repeater Field.
- Incorrect unique answer validation was triggered for valid values.
- Locker fields were clickable in the Gutenberg editor preview.
- The Form Builder settings screen had minor visual issues.
- A new installation of an addon may have caused a database error.

## [2.7.0] - 2023-12-11
### Changed
- Improved frontend validation performance.

### Fixed
- Unique answer restriction now applies only to completed form entries.
- Form Scheduling Start/End date and time were not properly validated.
...
## [2.5.0] - 2023-09-27
### IMPORTANT
- Support for PHP 5.6 has been discontinued. If you are running PHP 5.6, you MUST upgrade PHP before installing WPForms Form Locker 2.5.0. Failure to do that will disable WPForms Form Locker functionality.
- Support for WordPress 5.4 and below has been discontinued. If you are running any of those outdated versions, you MUST upgrade WordPress before installing WPForms Form Locker 2.5.0. Failure to do that will disable WPForms Form Locker functionality.

### Changed
- Minimum WPForms version supported is 1.8.4.
- Old deprecated `wpmu_new_blog` hook was replaced with the `wp_initialize_site` hook.

### Fixed
- The form was incorrectly locked when having payment entries.
