...
class Change_Login_URL {
    ...
    public function redirect_on_default_login_urls() {
        ...
        $options = get_option( ASENHA_SLUG_U );
        $custom_login_slug = $options['custom_login_slug']; // e.g. backend
        ...
        if ( isset( $_POST['log'] ) && ! empty( $_POST['log'] ) && isset( $_POST['pwd'] ) && ! empty( $_POST['pwd'] ) ) {
            ...
            if ( isset( $url_input_parts[1] ) 
                && 'wp-signup.php' == $url_input_parts[1] 
            ...
            // At this point, /wp-signup.php redirects to /wp-login.php?action=register&custom_login_slug and then to /wp-login.php?registration=disabled if registration is disabled
            elseif ( isset( $url_input_parts[1] )
