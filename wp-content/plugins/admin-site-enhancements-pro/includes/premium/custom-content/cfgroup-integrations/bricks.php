			...
			// Process field type logic
				...
				switch ( $field['type'] ) {
					...
					case 'relationship':
						...
						// At this point, $value is an indexed array of post IDs as strings, and the value of the first element is not a string of comma-separated post IDs. Let's convert to integers.
						if ( is_array( $value ) && ! empty( $value ) && false === strpos( $value[0], ',' ) ) {
							foreach ( $value as $index => $string ) {
								unset( $value[$index] );
								$value[$index] = intval( $string );
							}
